...
def main() -> int:
    ...
    while True:
        ...
        if len(hdr) < REQUEST_HEADER_LEN:
            print(
                f"[mock] short header: got {len(hdr)} bytes, expected "
                f"{REQUEST_HEADER_LEN}",
                file=sys.stderr,
            )
            return 1

        code_id, _max_iters, _sigma_sq, n_bits = struct.unpack(
        ...
        if len(payload) < 4 * n_bits:
            print(
                f"[mock] short LLR payload: got {len(payload)} bytes, "
                f"expected {4 * n_bits}",
                file=sys.stderr,
            )
            return 1
